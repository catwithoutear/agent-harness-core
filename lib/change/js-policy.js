...
export const JSON_EVIDENCE_DIRECTORIES = {
  "review-runs": {
    pattern: "review-runs/<run-id>/",
    runIdRegex: /^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/u,
    recordSchema: "schemas/review-run.schema.json",
...
export const COMMANDS = {
  ...
  migrate: "harness-change-doc migrate <change> --dry-run | --apply",
...
export function projectionPolicy() {
  return {
    ...
    json_evidence_directories: Object.fromEntries(
      Object.entries(JSON_EVIDENCE_DIRECTORIES).map(([name, value]) => [name, {
        pattern: value.pattern,
        run_id_regex: value.runIdRegex.source,
        record_schema: value.recordSchema,
...
export function managedJsonEvidenceDirectories() {
  return new Set(Object.keys(JSON_EVIDENCE_DIRECTORIES));
}
