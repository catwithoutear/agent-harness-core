...
const SUPPORTED_CLIENTS = new Set(["codex", "claude", "opencode", "omp", "zcode"]);
...
const HOOK_METADATA_FIELDS = new Set([
  "support",
  "event",
  "matcher",
  "effect",
...
const HOOK_METADATA_SUPPORTS = new Set(["native", "unsupported"]);
const HOOK_METADATA_EVENTS = new Set([
  "SessionStart",
  "UserPromptSubmit",
  "PreToolUse",
  "PermissionRequest",
...
const HOOK_METADATA_EFFECTS = new Set(["additionalContext"]);
...
function validateClientCapabilities(clientId, capabilities, errors) {
  ...
  validateHookCapabilities(clientId, capabilities.hooks, errors);
}

function validateHookCapabilities(clientId, hooks, errors) {
  if (hooks === undefined) {
    return;
  }
  const label = `client ${clientId} capabilities.hooks`;
  if (!hooks || typeof hooks !== "object" || Array.isArray(hooks)) {
    errors.push(`${label} must be an object`);
    return;
  }
  for (const [intent, value] of Object.entries(hooks)) {
    if (isCapabilityValue(value)) {
      continue;
    }
    validateHookMetadata(`${label}.${intent}`, value, errors);
  }
}

function validateHookMetadata(label, metadata, errors) {
  if (!metadata || typeof metadata !== "object" || Array.isArray(metadata)) {
    errors.push(`${label} must be a capability value or metadata object`);
    return;
  }
  for (const field of Object.keys(metadata)) {
    if (!HOOK_METADATA_FIELDS.has(field)) {
      errors.push(`${label}.${field} is not a supported Hook metadata field`);
    }
  }

  if (!HOOK_METADATA_SUPPORTS.has(metadata.support)) {
    errors.push(`${label}.support must be native or unsupported`);
  }
  if (metadata.event !== undefined && !HOOK_METADATA_EVENTS.has(metadata.event)) {
    errors.push(`${label}.event must be a supported native Hook event`);
  }
  if (
    metadata.matcher !== undefined &&
    metadata.matcher !== null &&
    typeof metadata.matcher !== "string"
  ) {
    errors.push(`${label}.matcher must be a string or null`);
  }
  if (metadata.effect !== undefined && !HOOK_METADATA_EFFECTS.has(metadata.effect)) {
    errors.push(`${label}.effect must be additionalContext`);
  }
  if (
    metadata.adapter_runtime_name !== undefined &&
    (typeof metadata.adapter_runtime_name !== "string" || metadata.adapter_runtime_name.length === 0)
  ) {
    errors.push(`${label}.adapter_runtime_name must be a non-empty string`);
  }

  if (metadata.support === "native") {
    if (!HOOK_METADATA_EVENTS.has(metadata.event)) {
      errors.push(`${label}.event is required for native Hook metadata`);
    }
    if (metadata.effect !== "additionalContext") {
      errors.push(`${label}.effect is required for native Hook metadata`);
    }
    if (
      typeof metadata.adapter_runtime_name !== "string" ||
      metadata.adapter_runtime_name.length === 0
    ) {
      errors.push(`${label}.adapter_runtime_name is required for native Hook metadata`);
    }
  }
}
