...
function verifyProjection(plan, options) {
  ...
  const records = plan.map((record) => {
    ...
    if (stateRecord.mode === "copy" || stateRecord.mode === "render") {
      const sourceHash = hashPath(record.source);
      if (sourceHash !== stateRecord.source_hash) {
        errors.push(`${record.target}: source hash mismatch`);
        return { ...record, status: "mismatch" };
      }
    }
    ...
    if (stateRecord.mode === "render") {
      ...
      const expectedContent = renderManagedContent(record);
      const actualContent = fs.readFileSync(record.target, "utf8");
      if (expectedContent === null || actualContent !== expectedContent) {
        errors.push(`${record.target}: rendered target does not match current renderer`);
        return { ...record, status: "mismatch" };
      }
    }
...
function readState(targetRoot) {
  ...
  try {
    ...
    return { error: `${statePath}: incompatible projection state (expected records array)` };
  } catch (error) {
    return { error: `${statePath}: cannot read projection state: ${error.message}` };
  }
}
