---
artifact: tasks-index
status: draft
tags: [implementation]
description: "Tasks index."
---
# Tasks

## Responsibility

This directory indexes tasks child documents.

## Child Index

| path | artifact | status | order | description |
|---|---|---|---|---|
| `slice-001-canonical-stage-order.md` | task-slice | draft | 001 | canonical-stage-order |
| `slice-002-template-and-user-guidance.md` | task-slice | draft | 002 | template-and-user-guidance |
| `slice-003-projection-and-final-verification.md` | task-slice | draft | 003 | projection-and-final-verification |
