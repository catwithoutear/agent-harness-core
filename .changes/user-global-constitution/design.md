...
artifact: design
status: draft
tags: [design, user-global-constitution, constitution-template, language-boundary]
description: "Design for the user-global agent constitution template."
---
...
| Area | Decision |
|---|---|
| Storage | Add a source template at `templates/user-global/AGENTS.md`, matching the intended user-global target file shape. |
| Template title | Keep `# Agent Constitution`. Do not rename it to `# User-Global Agent Instructions`. |
| README | Add English guidance to core `README.md`, because it is a long-lived repository asset. |
| Chinese README | Add `README_CN.md` as a full Chinese translation of `README.md`, and link to it from `README.md`. |
| AGENTS rule | Update core `AGENTS.md` to require synchronized updates when `README.md` or `README_CN.md` changes semantic content. Formatting-only or typo-only edits do not require paired changes unless they affect meaning. |
| Runtime projection | No projection change in the first slice. |
| Installer behavior | No automatic write to user-global `AGENTS.md` in the first slice. |
| Manifest | Do not add the template to `harness.manifest.json` in the first slice. It remains a documentation-only template. |
| Language | English for long-lived repo assets and agent-loaded content; Chinese for conversational plans, explanations, review reports, final answers, and the full `README_CN.md` translation. |
| Risk tier | Medium, because it affects reusable harness guidance but does not change runtime behavior. |
| Implementation-design pack | Not required: this is localized documentation/template work, no subsystem topology, lifecycle, state, concurrency, rollback, or dependency-order risk. |

## Candidate Template Body
...
The first slice should only add:

- `templates/user-global/AGENTS.md`;
- an English README section that points to the template and gives an explicit AI prompt for installing it;
- `README_CN.md` as a full Chinese translation, with `README.md` linking to it;
- a core `AGENTS.md` editing rule requiring README/README_CN synchronization for semantic content changes.

The first slice should not:

- write to `~/.agents`, `~/.codex`, or any user-global `AGENTS.md`;
- change hook behavior;
- change projector behavior;
- change `harness.manifest.json`;
- change projection tests;
- change source skills;
- edit projected runtime directories directly.
- leave `README_CN.md` partially translated.

## Review Focus

Review should check:

- whether the constitution text stays principle-level;
- whether the template keeps the accepted `# Agent Constitution` title;
- whether language boundaries are clear;
- whether `README_CN.md` is a faithful full translation of `README.md`;
- whether drift risk between `README.md` and `README_CN.md` is explicitly handled in `AGENTS.md` without over-triggering on non-semantic edits;
- whether no manifest/projector behavior is introduced by accident;
- whether the template avoids fixed project mechanisms;
- whether the implementation slice stays separate from existing uncommitted simplify-related changes.
