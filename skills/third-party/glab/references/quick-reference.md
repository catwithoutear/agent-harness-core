...
glab api projects/:id/merge_requests/123/discussions \
  ...
  --input /tmp/mr-discussion.json  # Add inline diff note
```
...
glab api projects/:id/merge_requests/123/discussions \
  --method POST -H 'Content-Type: application/json' \
  --input /tmp/mr-discussion.json  # Nested JSON payload
```
...
7. For inline MR diff notes, use `--input` JSON, not `-f position[...]`
