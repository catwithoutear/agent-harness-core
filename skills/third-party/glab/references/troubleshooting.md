...
3. If installed but not in PATH, add to PATH:
   ```bash
   # Find where glab is installed (use safer alternatives to find /)
   command -v glab  # Check if in PATH
   # Or check common installation locations
   ls -la /usr/local/bin/glab 2>/dev/null || ls -la /usr/bin/glab 2>/dev/null
   # Or use package manager (platform-specific)
   # macOS: brew list glab
   # Ubuntu/Debian: dpkg -L glab | grep bin
   # RHEL/CentOS: rpm -ql glab | grep bin
...
### Inline Comment Appears as Top-Level Discussion

**Symptom:**
You tried to comment on a specific diff line, but GitLab shows the note in the file
discussion area instead of attaching it to the code.

**Common cause:**
- Posting the discussion with `glab api -f position[...] ...`

That flag style works for flat fields, but it is not reliable for nested MR discussion
`position` objects. GitLab can accept the request and still create a plain
`DiscussionNote` rather than a code-anchored `DiffNote`.

**Reliable fix:**
1. Build a real JSON body:
   ```bash
   jq -n \
     --arg body "Inline review comment" \
     --arg base "$BASE_SHA" \
     --arg start "$START_SHA" \
     --arg head "$HEAD_SHA" \
...
2. Post it with an explicit JSON content type:
   ```bash
...
3. Verify the result is a real diff note by inspecting the returned payload or opening
   the MR diff view.
...
4. Auto-merge when pipeline passes:
   ```bash
   # Queues merge to happen automatically after pipeline succeeds
...
3. Open MR in browser (optional):
   ```bash
   # Open MR in browser (optional)
