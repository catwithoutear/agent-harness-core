...
# Filter by state (use flags, not --state)
glab mr list --merged       # or -M
glab mr list --closed       # or -c
glab mr list --all          # or -A
```
...
```bash
# View MR details in terminal (default)
glab mr view 123
...
# Add inline diff discussion on a specific line (reliable pattern)
jq -n \
  --arg body "Please keep retries alive after transient failures." \
  --arg base "$BASE_SHA" \
  --arg start "$START_SHA" \
  --arg head "$HEAD_SHA" \
...
# List closed issues
# List closed issues
glab issue list --closed       # or -c

# List all issues (open and closed)
glab issue list --all          # or -A
...
# Get CI job artifacts (glab 1.88.0+, replaces deprecated `glab ci artifact`)
glab job artifact <ref> <job-name>

# Download artifacts to specific path
glab job artifact <ref> <job-name> -p path/to/download
```
...
# POST with --field vs --raw-field
# --field: Type conversion (true/false/null/integers become JSON types)
# --raw-field: Always string, no conversion
glab api --method POST projects/:id/issues --raw-field title="Bug" --raw-field description="Details"

# POST with boolean/number (requires --field for type conversion)
glab api --method POST projects/:id/merge_requests --field merge_when_pipeline_succeeds=true --field squash=true

# POST with array (comma-separated values)
glab api --method POST projects/:id/issues --field labels="bug,critical,urgent"

# Nested JSON payloads: prefer --input with Content-Type
# This is the reliable way to create inline MR diff discussions.
glab api projects/:id/merge_requests/123/discussions \
  --method POST \
  -H 'Content-Type: application/json' \
  --input /tmp/mr-discussion.json
...
**Important:** avoid `glab api -f position[...]` for inline MR notes. That pattern can
degrade into a plain top-level `DiscussionNote` because the nested `position` object is
not preserved as intended. Use `jq` (or a prepared JSON file) plus `--input` instead.
